...
`/dashboard` MUST require `auth` + `role:STUDENT`. Lists subscribed classes as cards (title, description, #materials, #exams). MUST display "Exámenes disponibles" section: exams from subscribed classes where no StudentAttempt exists for the authenticated student, each with an "Iniciar examen" link to `student.exam.start`. MUST display "Exámenes completados" section: exams with a StudentAttempt for the student, showing "X / Y" where X=score_obtained and Y=exam.max_score. MUST display a "Mi perfil" link in the header/nav pointing to `route('profile.show')`. Empty state when zero subscriptions. Non-STUDENT denied. (Extended in exam-engine.)

(Previously: Dashboard did not include "Mi perfil" navigation link.)
...
#### Scenario: Dashboard displays Mi perfil link

- GIVEN an authenticated user on the dashboard
- WHEN the dashboard renders
- THEN a "Mi perfil" navigation link is visible pointing to `route('profile.show')`
