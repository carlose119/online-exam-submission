...
Meeting creation notifications require a supervised queue worker. A committed one-off meeting queues one summary per current enrolled, verified `STUDENT`; a recurring creation queues one series summary per recipient after every child is materialized, never one message per child. Eligibility is frozen inside the successful creation transaction, and dispatch occurs only after its outermost commit, so rollback queues nothing.

Queue delivery uses the generic mail failover and failed-job controls above. Workers retry a meeting notification up to three times before the normal failed-job path records metadata-only diagnostics; use `queue:retry` after correcting the transport. Each job contains one recipient context and only the class name, meeting title, first schedule, duration, occurrence count, and safe dashboard action. Meeting URLs, agendas, scores, answers/results, feed or verification/reset links, credentials, recipient lists, and unrelated model data are excluded.
